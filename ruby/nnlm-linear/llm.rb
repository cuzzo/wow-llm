...
class NNLM
  ...
  def activate(hidden_input)
    # Since ACTIVATION_CONSTANT is 1.0, we could return hidden_input directly.
    # The identity function (multiplying by 1.0) does nothing.
    #
    # This is simply an illustration that `activating` an input signal
    # Does some transformation.
    #
    # Importantly, we will want to transform the input signal from linear
    # to non-linear in the future, which will have many implications.
    #
    # For now, we will "transform" it with the identity function.
    scalar_multiply(ACTIVATION_CONSTANT, hidden_input) 
  end

  # TODO: KEEP THIS SIMPLE FOR NOW
  # In v2, when this changes to tanh(x), illustrate the cost-savings of using the output.
  # Additionally, show the cost savings in terms of `time ./train.rb`
  #
  # We need to calculate the derivative of the `activate` function for backpropagation.
  #
  # Let's define `activate` as function `f`, where:
  #   hidden_activation = activate(hidden_input)
  #   or mathematically: y = f(x) where:
  #     - x is the input (hidden_input)
  #     - y is the output (hidden_activation)
  #
  # For backpropagation, we need to know: "How much does the output `y` change 
  # when we slightly change the input `x`?" This is the derivative, written as `dy/dx`.
  #
  # Since `y = f(x)`, this derivative is also written as `f'(x) = dy/dx`.
  #
  # This function `deactivate` is responsible for calculating `f'(x)`.
  #
  # Notice the function signatures in code: `activate(input_x)` vs. `deactivate(output_y)`
  #
  # Even though `deactivate` here takes `y` (the output of `activate`) as input,
  # it computes `f'(x)`.
  #
  # How? Why?
  #
  # If our function is linear, the derivative (how much y changes), does not depend on `x`.
  # It is always changing at a constant (ACTIVATION_CONSTANT)!
  #
  # We could write this function as deactivate(hidden_output, hidden_input).
  # Or `derive(y, x)` - but we wouldn't need `x` (hidden_input).
  #
  # No matter what `x` is, the derivative (how much y changes) is always ACTIVATION_CONSTANT.
  #
  # Since `activate` is linear in this example (activate(x) = 1.0*x), the derivative of 
  # a linear function like so is a CONSTANT -- in this case, 1.0 (ACTIVATION_CONSTANT).
  #
  # In this case, we need `y` to know how long the resulting vector should be.
  #
  # General derivative example (using `deriv(...)` for "derivative of expression"):
  #
  # deriv(x^3) = 3x^2    -> non-linear (3x^2 is shaped like a parabola)
  # deriv(3x^2) = 6x     -> linear (6x is shaped like a sloped line)
  # deriv(6x) = 6        -> constant (6 is shaped like a flat line)
  #
  # This demonstrates that the derivative of a linear function (like 6x) 
  # WITH RESPECT TO its input (x) is a constant: 6.
  #
  # If above, in `activate`, our constant multiplier was 6.0 instead of 1.0
  # (i.e., activate(x) = 6.0*x), then the derivative f'(x) would be 6.0, and 
  # we would create a vector of all 6s here.
  #
  # We are getting the derivative of `activate`s output: `hidden_activation`
  # WITH RESPECT TO `activate`s input: `hidden_input`
  #
  # Mathematically, this is `dy/dx`. 
  #
  # So, matching the phrase "derivative of y WITH RESPECT TO x" -- `f'(x) = dy/dx`.
  #
  # Here:
  #   - x is the input TO the `activate` function => hidden_input
  #   - y is the output FROM the `activate` function => hidden_activation 
  #
  # It just so happens that for this case, we don't need the input (x).
  #
  # The calculated derivative (ACTIVATION_CONSTANT in this linear case) 
  # tells us how much `y` changes per unit change in `x`.
  def deactivate(hidden_activation)
    ...
    #
    # It's derivative is 1.0.
    #
    # If `activate` transformed the values in a non-linear way, say:
    #
    # def activate(x) 
    #   x**2
    # end
    #
    # Then `activate`s derivative would be: 2x
    #
    # hidden_input.map { |x| x * 2.0 }
    #
    # This is *non-invertible*. We can't do:
    #
    # hidden_activation.map { |y| sqrt(y) * 2.0 }
    #
    # If `x` was -3, then deactivate(x) should be -6.
    #
    # activate(-3) = -3*-3 = 9
    #
    # If we used the output (9) and attemtped to get the sqrt (3):
    # We'd get 6 instead of -6.
    #
    # However, we will only use activation functions where the derivative
    # can be expressed in terms of the output `y` - and usually where it
    # is computationally less expensive to do so.
    #
    # Which is why this signature takes y instead of x.
  end
  ...
  def backward(context_indices, target_index, forward_pass_data)
    ...
    d_hidden_input = multiply_elementwise(d_hidden_input_signal, deactivate(hidden_activation))
