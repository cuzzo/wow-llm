...
class TokenLLM
  ...
  # length: The length of the output text to generate
  # temp: The temperature - see temper_options
  # k: The score to give each token in the vocab
  # cache_size: The size of the cache
  # cache_weight: The weight to give the cache
  def generate(prompt, length = 100, temp = 1.0, k = 0.5, cache_size = 30, cache_weight = 0.25)
    ...
    raise "Prompt must be at least #{@context_size} characters long" if generated_tokens.length < @context_size

    # Get the last context_size characters
    current_context = generated_tokens[(-@context_size)..]
    cache = current_context.slice([-cache_size, -@context_size+1].max, @context_size)

    puts "Generating #{length} characters..."
    length.times do
      options = next_options(current_context, temp, k, cache, cache_weight)
  ...
  def next_options(context, temp, k, cache, cache_weight)
    context
      .then { |context| interpolate_options(context) }
      .then { |options| blend_cache(options, cache, cache_weight) }
      .then { |options| temper_options(options, temp) }
  ...
  def blend_cache(options, cache, cache_weight)
    len = cache.size
    exist_weight = 1.0 - cache_weight
    options.transform_values! { |cnt| cnt * exist_weight }
    cache.each_with_index do |token, _idx|
      options[token] ||= 0
      options[token] += (1 / len) * cache_weight
    end
    options
  end
