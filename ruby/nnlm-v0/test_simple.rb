...
class TestSimpleNNLM < Minitest::Test
  ...
  def setup
    ...
    puts "\nSETUP: Using updated embeddings: #{embeddings.inspect}" # Add confirmation

    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    # !! UPDATED W_h and b_h for the slope steepness test demonstration !!
    # !! Goal: For input [0.0, 1.0] (index 0), make z0=0 and z1=2       !!
    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    # z0 = w10 + b0 => 0.5 + (-0.5) = 0
    # z1 = w11 + b1 => 1.5 + 0.5 = 2
    ...
    puts "\nSETUP: Using W_h = #{w_h.inspect}"
    puts "SETUP: Using b_h = #{b_h.inspect}"
    ...
    @initial_embeddings = deep_copy(embeddings)
    @initial_W_h = deep_copy(w_h)
    @initial_b_h = deep_copy(b_h)
    @initial_W_o = deep_copy(w_o)
    @initial_b_o = deep_copy(b_o)
  ...
  def test_forward_input_layer_is_correct_embedding
    ...
    puts "Forward Input Layer OK (Matches new embedding)"
  end
  ...
  def test_forward_hidden_activation_sign_matches_input_sign
    puts "\n--- Testing Forward Hidden Activation Sign ---"
    ...
    contexts_to_test.each do |context_indices|
      puts "\nTesting context: #{context_indices.inspect} ('#{@nnlm.ix_to_word[context_indices[0]]}')"
      ...
      puts "  Input Layer (@initial_embeddings[#{context_idx}]): #{input_layer.inspect}"
      puts "  Expected Hidden Input (pre-tanh): #{expected_hidden_input_pre_tanh.map { |v| v.round(6) }.inspect}"
      ...
      puts "  Actual Hidden Activation (post-tanh): #{actual_hidden_activation_post_tanh.map { |v| v.round(6) }.inspect}"
      ...
      puts "  Sign check PASSED for context #{context_indices}"
    end
    puts "\n--- Hidden Activation Sign Test Complete ---"
  end

  def test_forward_hidden_slope_is_steepest_near_zero_input
    puts "\n--- Testing Forward Hidden Activation Slope Steepness ---"
    ...
    puts "Testing context: #{context_indices.inspect} ('#{@nnlm.ix_to_word[context_indices[0]]}')"
    ...
    puts "  Input Layer (Initial): #{input_layer.inspect}"
    puts "  Expected Hidden Input (pre-tanh): #{expected_hidden_input_pre_tanh.map { |v| v.round(6) }.inspect}"
    ...
    puts "  Actual Slopes (1 - activation^2): #{actual_slopes.map { |s| s.round(6) }.inspect}"
    ...
    puts "  Slope steepness check PASSED for context #{context_indices}"
    puts "--- Hidden Activation Slope Test Complete ---"
  end
end
