...
class TokenLLMTest < Minitest::Test
  TOKEN_A = 1
  TOKEN_B = 2
  TOKEN_C = 3

  def setup
    @llm = TokenLLM.new(3)  # Default trigram model
    @llm4 = TokenLLM.new(4) # Default quadgram model
  end
  ...
  def test_training
    spp = "S"
    @llm4.train("h e l l o\n\nw o r l d\n\nh e y")

    def context_key(id)
      context = id.chars.map { |c| char_key(c) }
      @llm4.send(:context_id, context)
    end

    def char_key(id)
      id = id == "S" ? "[PARAGRAPH]" : id
      @llm4.instance_variable_get(:@token_to_id)[id]
    end

    expected_model = {
      # Bi-grams
      context_key("h") => { char_key("e") => 2},
      context_key("e") => { char_key("l") => 1, char_key("y") => 1},
      context_key("l") => { char_key("l") => 1, char_key("o") => 1, char_key("d") => 1},
      context_key("o") => { char_key(spp) => 1, char_key("r") => 1},
      context_key(spp) => { char_key("w") => 1, char_key("h") => 1},
      context_key("w") => { char_key("o") => 1},
      context_key("r") => { char_key("l") => 1},
      context_key("d") => { char_key(spp) => 1},

      # Tri-grams
      context_key("he") => { char_key("l") => 1, char_key("y") => 1 },
      context_key("el") => { char_key("l") => 1 },
      context_key("ll") => { char_key("o") => 1 },
      context_key("lo") => { char_key(spp) => 1 },
      context_key("o" + spp) => { char_key("w") => 1 },
      context_key(spp + "w") => { char_key("o") => 1 },
      context_key("wo") => { char_key("r") => 1 },
      context_key("or") => { char_key("l") => 1 },
      context_key("rl") => { char_key("d") => 1 },
      context_key("ld") => { char_key(spp) => 1 },
      context_key("d" + spp) => { char_key("h") => 1 },
      context_key(spp + "h") => { char_key("e") => 1 },

      # Quad-grams
      context_key("hel") => { char_key("l") => 1 },
      context_key("ell") => { char_key("o") => 1 },
      context_key("llo") => { char_key(spp) => 1 },
      context_key("lo" + spp) => { char_key("w") => 1 },
      context_key("o" + spp + "w") => { char_key("o") => 1 },
      context_key(spp + "wo") => { char_key("r") => 1 },
      context_key("wor") => { char_key("l") => 1 },
      context_key("orl") => { char_key("d") => 1 },
      context_key("rld") => { char_key(spp) => 1 },
      context_key("ld" + spp) => { char_key("h") => 1 },
      context_key("d" + spp + "h") => { char_key("e") => 1 },
      context_key(spp + "he") => { char_key("y") => 1 },
    }

    assert_equal expected_model, @llm4.model
    assert_equal 9, @llm4.vocab.size  # 10 unique words in "h e l l o\n\nw o r l d\n\nh e y"
  ...
  # Test context ID packs 4 integers as expected
  def test_context_id_properly_encoded
    # Test that the same context always produces the same ID
    context = [1, 2, 3, 4]
    id = @llm.send(:context_id, context)
  ...
  def test_load_model
    ...
    # If the test string is change from "world hello world" to "hello world",
    # The loaded model will not include "world" in the vocab.
    #
  ...
  def test_interpolate_full_match
    ...
    current_context = llm4.send(:tokenize, "b c d")
    # @model["bcd"] => {" " => 1, "2" => 2}
    # @model["cd"] => {" " => 1, "2" => 2}
    # @model["d"] => {" " => 1, "2" => 2, "x" => 1}

    weights = llm4.send(:weights)
    k_sp = llm4.instance_variable_get(:@token_to_id)["[PARAGRAPH]"]
    k_2 = llm4.instance_variable_get(:@token_to_id)["2"]
    ...
    assert_equal expected_options.keys, actual_options.keys
    assert_in_delta expected_options[k_sp], actual_options[k_sp], 1e-9
    assert_in_delta expected_options[k_2], actual_options[k_2], 1e-9
    assert_in_delta expected_options[k_x], actual_options[k_x], 1e-9
  end
  ...
  def test_interpolate_partial_match
    ...
    current_context = llm4.send(:tokenize, "a c d")
    # @model["bcd"] => {" " => 1, "2" => 2}
    # @model["cd"] => {" " => 1, "2" => 2}
    # @model["d"] => {" " => 1, "2" => 2, "x" => 1}
  ...
  def test_interpolate_no_match
    ...
    current_context = llm4.send(:tokenize, "a c y")
    # @model["acy"] => {}
    # @model["cy"] => {}
    # @model["y"] => {}
