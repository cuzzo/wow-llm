require "spec"
require "../src/llm" 
require "../src/tokeniza_bindings"

class RustTokenizerMock < RustTokenizerInterface
  @next_token_id : UInt32
  ...
  def initialize()
    ...
    @token_to_id = Hash(String, Token).new(0)
  end

  def encode(text : String) : Array(UInt32)
     text
      .split(/\s+/)
      .map { |token| get_token_id(token).to_u32 }
  end

  def decode(token_ids : Array(UInt32)) : String
    token_ids
      .map { |id| @id_to_token[id.to_u16] }
      .join(" ")
  end
  ...
  def get_token_id(token : String)
    unless @token_to_id.has_key?(token)
      @token_to_id[token] = @next_token_id.to_u16
      @id_to_token[@next_token_id.to_u16] = token
      @next_token_id += 1
...
describe NGramLLM do
  ...
  tokenizer = RustTokenizerMock.new()
  ...
  context "#interpolate_options" do
    ...
    it "calculates interpolated scores with full context match" do
      ...
      k_sp = tokenizer.@token_to_id["*"]
      k_2 = tokenizer.@token_to_id["2"]
      k_x = tokenizer.@token_to_id["x"]
  ...
  context "context_shift" do
    ...
    it "shifts out of bounds and adds" do
      ...
      shifted_cc_id = llm.context_shift(cc_id, 3_u16)
      expected_cc_id = llm.context_id([2_u16, 3_u16])
      shifted_cc_id.should eq(expected_cc_id)
    end
  end
  ...
  context "train" do
    ...
    it "backoffs properly" do
      ...
      k_1 = tokenizer.@token_to_id["1"]
      k_2 = tokenizer.@token_to_id["2"]
      k_3 = tokenizer.@token_to_id["3"]
      k_4 = tokenizer.@token_to_id["4"]
